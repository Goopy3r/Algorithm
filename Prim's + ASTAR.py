...
ASTAR_SOLVING_PARTICLE = '#FF00FF'  # Changed to a different color for A*
...
def solve_maze_astar(grid, start_pos, end_pos):
    width = len(grid)
    height = len(grid[0])
    ...
    priority_queue = [(heuristic(start_pos, end_pos), 0, start_pos)]
    visited = set()
    came_from = {}
    solving_states = []
    ...
    g_score = [[float('inf') for _ in range(height)] for _ in range(width)]
    g_score[start_x][start_y] = 0

    f_score = [[float('inf') for _ in range(height)] for _ in range(width)]
    f_score[start_x][start_y] = heuristic(start_pos, end_pos)

    while priority_queue and not solution_found:
        _, current_g, current_pos = heapq.heappop(priority_queue)
                ...
                if (direction == 'N' and not grid[x][y].walls['N']) or \
                   ...
                   (direction == 'W' and not grid[x][y].walls['W']):
                    ...
                    if tentative_g_score < g_score[nx][ny]:
                        ...
                        f_score[nx][ny] = tentative_g_score + heuristic((nx, ny), end_pos)
                        
                        heapq.heappush(priority_queue, (f_score[nx][ny], tentative_g_score, (nx, ny)))
...
def main():
    ...
    combo = {
        ...
        "solving_algo": "solve_maze_astar",
        "gen_name": "Prim's Algorithm",
        "solving_name": "A* Algorithm",
        "gen_particle_color": PRIM_GENERATION_PARTICLE,
        "solving_particle_color": ASTAR_SOLVING_PARTICLE,
        "output_file": "prim_astar_maze.mp4"
    }
